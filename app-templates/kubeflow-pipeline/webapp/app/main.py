...
def summary():
  ...
  if request.method == 'POST':
    print("*** Using model %s for predictions ***", model_name)
    ...
    print("Running predict in line 120...")
    outputs = serving_utils.predict([issue_text], problem, request_fn)
    outputs, = outputs
    output, score = outputs
    tf.logging.info("output: %s : score %s", output, score)
    print("output: %s : score %s", output, score)
